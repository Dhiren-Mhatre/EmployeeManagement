        ...
        <Box display="flex" alignItems="center">
          <IconButton onClick={() => handleEditLeave(row._id)}>
            <ModeEditOutlineOutlinedIcon
              style={{ color: "#784B84", fontSize: 26 }}
            />
          </IconButton>
          <IconButton
            onClick={() =>
              handleDelete(row._id, row.name, row.type, row.unit, row.status)
            }
          >
    ...
    try {
      if (!searchQuery) {
        ...
        const filteredLeaveData = leaveData.filter((leave) => {
          ...
          return (
            leaveName.includes(query) ||
            leaveType.includes(query) ||
            leaveUnit.includes(query)
          );
        });
    ...
    <div>
      <Box m="40px 0 0 0">
        ...
        <Dialog open={editDialogOpen} onClose={handleDialogClose}>
          <DialogTitle>
            {editedLeave._id ? "Edit Leave Type" : "Add Leave Type"}
          </DialogTitle>
          <DialogContent sx={{ padding: "20px" }}>
            <TextField
              ...
              onChange={(e) =>
                setEditedLeave({ ...editedLeave, name: e.target.value })
              }
            ...
            <TextField
              ...
              onChange={(e) =>
                setEditedLeave({ ...editedLeave, unit: e.target.value })
              }
              sx={{ marginBottom: "16px" }}
            />
            ...
            <Button
              ...
              onClick={() =>
                setEditedLeave({
                  ...editedLeave,
                  status:
                    editedLeave.status === "active" ? "inactive" : "active",
                })
              }
              sx={{
                marginBottom: "16px",
                backgroundColor:
                  editedLeave.status === "active" ? "#4CAF50" : "#D21F3C",
                color: "#fff",
              }}
            >
