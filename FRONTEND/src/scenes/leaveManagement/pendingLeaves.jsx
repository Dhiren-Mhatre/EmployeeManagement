...
const PendingLeaves = () => {
  ...
  const handleApproveRequest = async () => {
    try {
      const response = await axiosInstance.patch(
        `http://localhost:5000/api/approve_requests/${selectedRequestId}`
      );
  ...
  const handleRejectRequest = async () => {
    try {
      const response = await axiosInstance.patch(
        `http://localhost:5000/api/reject_requests/${selectedRequestId}`
      );
      if (response.status === 200) {
        setLeaveRequests(
          leaveRequests.map((request) =>
  ...
  return (
    <div>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Are you sure you want to approve this leave request? The action
            cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Are you sure you want to reject this leave request? The action
            cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
