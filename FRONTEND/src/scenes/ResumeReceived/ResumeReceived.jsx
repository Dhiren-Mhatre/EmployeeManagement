...
const ResumeReceived = () => {
  ...
  const [emailDataForConfirmation, setEmailDataForConfirmation] =
    useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstance.post(
        "https://employeemanagement-backend-rkhb.onrender.com/api/send-email",
        emailData
      );
    ...
    const fetchData = async () => {
      try {
        const response = await axiosInstance.get(
          `https://employeemanagement-backend-rkhb.onrender.com/api/applied-applicants/${selectedJob}`
        );
  ...
  const fetchJobDetails = async (jobId) => {
    try {
      const response = await axiosInstance.get(
        `https://employeemanagement-backend-rkhb.onrender.com/api/get_job/${jobId}`
      );
    ...
    try {
      if (selectedAction === "shortlist" || selectedAction === "unshortlist") {
        ...
        await axiosInstance.post(
          `https://employeemanagement-backend-rkhb.onrender.com/api/${selectedAction}/${selectedApplicantId}`,
          { jobId: selectedJob }
        );
      ...
      console.error(
        `Error ${
          selectedAction === "shortlist" ? "shortlisting" : "unshortlisting"
        } applicant:`,
        error
      );
    }
  ...
  const filteredApplicantData = applicantData.filter(
    ...
    <Box m="20px">
      ...
      {jobDetails && (
        ...
        >
          ...
          <Typography>
            <strong>Salary Type:</strong> {jobDetails.salaryType}
          </Typography>
          <Typography>
            <strong>Job Location:</strong> {jobDetails.jobLocation}
          </Typography>
          <Typography>
            <strong>Posting Date:</strong> {jobDetails.postingDate}
          </Typography>
          <Typography>
          ...
          <Typography>
            ...
            {jobDetails.skills.map((skill) => skill.label).join(", ")}
          </Typography>
        </div>
      )}
          ...
          columns={[
            ...
            { field: "email", headerName: "Email", width: 250 },
            {
              ...
              renderCell: (params) => (
                <a
                  href="#"
                  onClick={() => handleCVClick(params.value)}
            ...
            },
            {
              ...
              renderCell: (params) => (
                <EmailIcon
                  ...
                  onClick={() => handleEmailSend(params.row.id)}
                />
              ),
            },
          ]}
